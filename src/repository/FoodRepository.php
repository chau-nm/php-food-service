...
class FoodRepository implements RepositoryInterface
{
    ...
    public function findByKey(int|string $key): ?Food
    {
        $query = "SELECT " . self::FULL_ATTRIBUTES . " FROM " . self::TABLE_NAME . " WHERE `uuid` = ?";
        $prepare = $this->db->prepare($query);
        $prepare->execute([$key]);
        $result = $prepare->fetch(\PDO::FETCH_ASSOC);
        return $result ? Food::fromQueryResult($result) : null;
    }
    ...
    public function findAll(): array
    {
        ...
        return Food::fromQueryArrayResult($result);
    }
    ...
    public function findBy(array $where): array
    {
        ...
        return Food::fromQueryArrayResult($results);
    }
